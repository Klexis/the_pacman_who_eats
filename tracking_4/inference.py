    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.beliefs:
            positions = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            # print(positions)
            # print(len(positions))
            for p in positions:
                allPossible[p] += self.beliefs[pos]*positions[p]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    beliefs[pos] += oldBeliefs[pos] * emissionModel[trueDistance]
    ...
    def observeState(self, gameState):
        ...
        if beliefs.totalCount() == 0:
            ...
            for x in range(len(self.particles)):
                self.particles[x] = util.sample(beliefs)
